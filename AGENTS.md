...
`src/` contains the application code. Use `src/app/` for UI screens, components, hooks, routes, contexts, and shared app utilities; use `src/db/` for Dexie/IndexedDB schema, validators, analytics, AI, recurring processing, export/import, and CRUD operations; use `src/lib/` for ML classifier and AI receipt parsers; use `src/services/` for AI client and context builder. Shared styles live in `src/styles/`, test setup in `src/test/setup.ts`, static assets and ML models in `public/`, and maintenance scripts in `scripts/`. Reference docs such as `ARCHITECTURE.md`, `DATABASE_SCHEMA.md`, `DATABASE_FIELDS.md`, and `APP_STACK_AND_FEATURES.md` should be updated when behavior or data contracts change.
...
- **Receipt scanner:** multi-engine (QR → Gemini Vision → Claude Vision → Tesseract.js OCR), all lazy-loaded
