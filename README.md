...
- Итоги по дням — чистый баланс (`+N ₽` / `−N ₽`) в заголовке каждой группы дат
- Экспорт в CSV при активных фильтрах — кнопка рядом со счётчиком «Найдено», экспортирует текущую выборку
...
- Тоггл «Обязательный расход» при создании/редактировании категории-расхода (влияет на метрику Health Score)
- Сортировка категорий по частоте использования (от наиболее используемых)
...
- При достижении 100% появляется кнопка «Завершить цель» (переводит в `isActive: false`)
- Завершённые цели вынесены в отдельную секцию «Достигнуто»
...
- Отдельный экран «Регулярные платежи» (ранее назывался «Повтор»)
...
- Настройки уведомлений: 4 переключателя (Push, Бюджеты, Цели, Платежи) — состояние сохраняется в localStorage
...
- **Прогноз баланса на 3 месяца** — AreaChart с проекцией cash flow
- **What-If симулятор** — слайдеры (Radix UI) для сокращения топ-4 категорий; пресеты «Правило 50/30/20», «Режим экономии −20%», «Сброс»; моментальный расчёт экономии в месяц/год и новой нормы сбережений
- **Эффект малых привычек** — выявляет регулярные траты (threshold: кол-во транзакций ≥ period_days/3), показывает проекцию в день/месяц/год и эквивалент в месяцах расходов

#### Вкладка «Сравнения»
- **Расходы по дням недели** — BarChart Пн–Вс, пиковый день выделен оранжевым
- **Год к году** — сравнение текущего месяца с аналогичным периодом прошлого года, горизонтальный BarChart по категориям
- **Бюджет vs Факт** — прогрессбары с цветовым статусом (зелёный < 80% / жёлтый < 100% / красный перерасход)
- **Календарный heatmap** — весь год по неделям, тепловая палитра amber→orange→red, подписи месяцев
- **Топ-10 крупнейших транзакций** — нумерованный список с категорией, комментарием, суммой и датой
- **Декомпозиция тренда** — STL-разложение: тренд + сезонность + остаток (recharts ComposedChart)

#### Вкладка «Здоровье»
- **Financial Health Score** — полукруговой gauge, оценка 0–100 с 6 метриками: норма сбережений, обязательные расходы, регулярность доходов, резервный фонд, диверсификация (HHI), долговая нагрузка
- **Metric drilldown** — тап на любую метрику раскрывает инлайн-карточку (AnimatePresence) с объяснением формулы и 2–3 советами «как улучшить»
- **Прогноз достижения целей** — GoalsTimeline: прогрессбар каждой цели + ETA на основе текущей нормы сбережений
- **План улучшения** — автоматически генерируемые рекомендации по слабым метрикам
- **Детектор проблем** — алерты типа danger/warning (критически низкие сбережения, нет бюджетов, нерегулярные доходы)

#### Вкладка «AI-инсайты»
- **Инсайты** — авто-карточки из `useAIInsights` по типам alert / warning / tip / positive с иконками и цветовым кодированием
- **Полный AI-чат** — bubble UI с историей сообщений в сессии, 5 suggestion pills, кнопка очистки истории; онлайн-режим через OpenRouter, офлайн-фолбек на локальный NLP-движок; метка «офлайн-ответ» на сообщениях
- **Еженедельный AI-отчёт** — генерируется по запросу через `/api/ai-chat`; markdown-рендеринг с **жирным**, ## заголовками и списками; кнопка «Обновить отчёт»

#### Period picker
Все три вкладки реагируют на переключатель **Месяц / Квартал / Год** — при смене периода данные перезапрашиваются через `getPeriodRange(period)` из `src/lib/period.ts`.
...
│   │   │   ├── premium/            # Premium-компоненты
│   │   │   │   ├── AIInsightsPanel.tsx    # AI-инсайты + полный чат + еженедельный отчёт
│   │   │   │   ├── CalendarHeatmap.tsx    # Годовой heatmap (amber→red палитра, подписи месяцев)
│   │   │   │   ├── ComparativeTab.tsx     # Вкладка Сравнения
│   │   │   │   ├── HealthTab.tsx          # Вкладка Здоровье с drilldown метрик
│   │   │   │   ├── PredictiveTab.tsx      # Вкладка Прогнозы
│   │   │   │   ├── WhatIfSimulator.tsx    # What-If симулятор (Radix Slider + пресеты)
│   │   │   │   ├── HealthScoreGauge.tsx   # Полукруговой gauge финансового здоровья
│   │   │   │   ├── TrendDecomposition.tsx # STL-декомпозиция
│   │   │   │   ├── DayOfWeekChart.tsx     # BarChart расходов по дням недели
│   │   │   │   ├── TopTransactions.tsx    # Топ-10 крупнейших транзакций
